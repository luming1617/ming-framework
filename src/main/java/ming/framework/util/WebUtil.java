...
public class WebUtil {
     ...
     * 普通文本
     */
    public static final String RESPONSE_CONTENT_TYPE_PLAIN = "text/plain";
     ...
     * 文件（二进制流方式，不知道下载文件类型）
     */
    public static final String RESPONSE_CONTENT_TYPE_FILE = "application/octet-stream";
    ...
    public static void writeHTML(HttpServletResponse response, Object data) {
        try {
            // 设置响应头
            // 指定内容类型为 HTML 格式
            response.setContentType(RESPONSE_CONTENT_TYPE_HTML);
            // 防止中文乱码
            response.setCharacterEncoding(FrameworkConstant.UTF_8);
            // 向响应中写入数据
            PrintWriter writer = response.getWriter();
    ...
    public static void downloadFile(HttpServletResponse response, String filePath) {
        ...
        try {
            downloadedFileName = new String(originalFileName.getBytes("GBK"), "ISO8859_1");
        } catch (UnsupportedEncodingException e) {
            logger.error(e.getMessage(), e);
        }

        response.setContentType(RESPONSE_CONTENT_TYPE_FILE);
